...
var files=fs.readdirSync(dir).filter(function(f){ return /\.(xlsx|xls)$/i.test(f); }).sort();
if(!files.length){ console.error("No hay .xlsx/.xls en "+dir); process.exit(1); }

var totUsadas=0, totLimpias=0, totMed=0;
files.forEach(function(f){
  var wb=XLSX.read(fs.readFileSync(path.join(dir,f)),{type:"buffer"});
  var det=core.normalizar(wb, f, XLSX, {});
...
console.log("\n──────── RESUMEN ────────");
console.log("Ficheros: "+files.length+" · mediciones totales: "+totMed);
