...
(function (root) {
    ...
    for(var i=start;i<grid.length;i++){
      ...
      if(code){
        if(isCaps(desc) && qty===null){ stats.titulo++; partida=null;
          if(generica){
            if(esRomano(code)){ capitulo=desc; subcap=""; romanoVisto=true;     // título romano (I., II., III.) = capítulo
              pushEst({codigo:code,nat:"Capitulo",ud:"",partida:desc,cantidad:""}); }
            else if(romanoVisto){ subcap=desc;                                   // bajo un capítulo romano, el título arábigo (0., 1.…) es subcapítulo
              pushEst({codigo:code,nat:"Subcapitulo",ud:"",partida:desc,cantidad:""}); }
            else { capitulo=desc; subcap="";                                     // hoja genérica sin nivel romano: el título sube a capítulo (el nombre de hoja no cuenta)
              pushEst({codigo:code,nat:"Capitulo",ud:"",partida:desc,cantidad:""}); }
          }
  ...
  function costesFormulados(estructura, startRow){
    ...
    function partida(rw){
      ...
      r[31]={f:"AE"+R+"*E"+R}; r[37]={f:"SUM(AH"+R+",AI"+R+",AJ"+R+",AK"+R+")*E"+R};
      r[38]={f:"SUM(AC"+R+",AF"+R+",AG"+R+",AL"+R+")"}; r[39]={f:"O"+R+"+R"+R+"+Y"+R+"+AM"+R};
