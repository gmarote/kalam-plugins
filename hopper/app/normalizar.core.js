...
(function (root) {
  ...
  // código "fuerte" e inequívoco: 3+ segmentos (2.1.1) o prefijo de letra (E.1) —
  // las cantidades nunca tienen dos puntos, así que esto distingue código de cantidad.
  function isStrongCode(s){
    if(!isStructCode(s)) return false;
    return s.split(".").length>=3 || /^[A-Za-z]{1,4}[.\d]/.test(s);
  }
  ...
  function autoDetect(grid){
    ...
    var met=[]; for(var c=0;c<N;c++) met[c]={strong:0,num:0,numNZ:0,unit:0,textLen:0,textN:0};
    var seen=0;
    for(var r=start;r<grid.length && seen<400;r++){
      ...
      for(var k=0;k<N;k++){
        ...
        if(isStrongCode(s)) met[k].strong++;
        var nu=toNum(s);
    ...
    pick("code", function(m){ return m.strong>=2 ? m.strong : 0; });          // jerarquía real
    ...
    var familia = (cols.code!=null && met[cols.code] && met[cols.code].strong>=2) ? "codigo" : "formato";
