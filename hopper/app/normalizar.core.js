...
(function (root) {
  ...
  function avisosDe(inf, medRows){
    ...
    if(z>0){
      if(z>=np*0.5)   // la mayoría a 0 -> la columna de cantidad es sospechosa
        a.push(z+" de "+np+" partidas con cantidad 0 — puede que la columna "+colLetra(c.qty)+" no sea la de cantidad.");
      else            // unas pocas -> es normal, solo se informa (sin conclusión)
        a.push(z+" partida"+(z>1?"s":"")+" con cantidad 0 (probablemente sin medir todavía).");
    }
    return a;
  }
